...
func getVal(a *[size][size]float64, p Pair) float64 {
	return (*a)[p.row][p.col]
}

func setVal(a *[size][size]float64, p Pair, val float64) {
	(*a)[p.row][p.col] = val
}

func isOut(p Pair) bool{
	return p.row < 0 || p.row >= size || p.col < 0 || p.col >= size
}

func diamond(a* [size][size]float64, p Pair, l int){
...
func square(a* [size][size]float64, p Pair, l int){
...
func update(a *[size][size]float64, p Pair, l int) {
		//fmt.Println(l, p)
...
func printArray(a[size][size] float64) {
	fmt.Print("[")
	for i := 0; i < size-1; i++ {
		fmt.Print("[")
		for j:= 0; j < size-1; j++ {
